...
cursor.execute("CREATE TABLE IF NOT EXISTS student_details (name VARCHAR(20), year VARCHAR(10), course VARCHAR(20))")
...
class details:
    def __init__(self, other):
        ...
        if x == 0:
            ...
            self.year = 'BE'
        self.course = other.v  # Use the selected course directly

        # Insert data into the database
        sql = "INSERT INTO student_details (name, year, course) VALUES (%s, %s, %s)"
        val = (self.name, self.year, self.course)
...
class window1:
    def __init__(self):
        ...
        self.title2 = Label(self.f2, text="Choose one of the following options", font=('Urbanist', 16, 'bold'),
                            bg='yellow', fg='black')
        self.title2.pack()

        self.title = Label(self.f1, text="Student Management System", font=('Urbanist', 16, 'bold'), bg='light blue',
                           fg='black')
        self.b1 = Button(self.f1, text="ADD", command=self.add, bg='pink', fg='black', height=2, width=20,
                         activebackground='red', activeforeground='white')
        self.b2 = Button(self.f1, text="SEARCH", command=self.search, bg='pink', fg='black', height=2, width=20,
                         activebackground='red', activeforeground='white')
        self.b3 = Button(self.f1, text="EXIT", command=quit, bg='pink', fg='black', height=2, width=20,
                         activebackground='red', activeforeground='white')
...
# Second Window for taking details and passing it to details object.
class window2:
    def __init__(self):
        self.root = Tk()
        ...
        self.b1 = Button(self.f1, text="ADD", command=self.add_to_db)
...
# Third window for searching details from the database based on name
class window3:
    def __init__(self):
        self.root = Tk()
    ...
    def search(self):
        ...
        name_to_search = self.e1.get().lower()

        # Query the database for the given name
        sql = "SELECT name, year FROM student_details WHERE LOWER(name) = %s"
        cursor.execute(sql, (name_to_search,))
        records = cursor.fetchall()
        for record in records:
            str_to_display = f"{record[0]}\t{record[1]}"
            self.t.insert(END, str_to_display + '\n')
